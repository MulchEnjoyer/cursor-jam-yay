...
class RetroEffectImpl extends Effect {
  constructor() {
    ...
    super("RetroEffect", ditherFragmentShader, {
      // react-three/postprocessing expects a `Map<string, Uniform>` type,
      // but `three`'s typings vary across versions. Cast is safe: the
      // underlying objects are real THREE.Uniform instances.
      uniforms,
    });
  }
...
function DitheredWaves({
  ...
  // Stable uniforms object to keep React refs lint happy.
  ...
  // Track mouse without attaching pointer handlers to the canvas (so it never blocks UI clicks).
  useEffect(() => {
  ...
  useFrame(({ clock }) => {
    ...
    // Uniforms are intentionally mutated each frame.
...
export default function Dither({
  ...
  const effectiveDisableAnimation = disableAnimation || reducedMotion;

  const normalizedWaveColor = useMemo(() => {
    const arr = Array.isArray(waveColor) ? waveColor : [0.5, 0.5, 0.5];
    return [arr[0] ?? 0.5, arr[1] ?? 0.5, arr[2] ?? 0.5] as [number, number, number];
  }, [waveColor]);
